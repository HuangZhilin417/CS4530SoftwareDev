...
export async function importGrades(gradesToImport: ImportTranscript[]): Promise<Transcript[]> {
  ...
  const studentIDs = [];
  for (const transcripts of gradesToImport) {
    const newStudent = await client.addStudent(transcripts.studentName);
    studentIDs.push(newStudent.studentID);
    for (const courseGrade of transcripts.grades) {
