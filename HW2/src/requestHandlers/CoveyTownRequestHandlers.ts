...
export function conversationAreaCreateHandler(
  ...
  const s = coveyTownController?.getSessionByToken(_requestData.sessionToken);

  if (!s || !coveyTownController) {
    return {
      isOK: false,
      response: {},
  ...
  if (!coveyTownController.addConversationArea(_requestData.conversationArea)) {
    return {
      isOK: false,
      response: {},
      message: `Unable to create conversation area ${_requestData.conversationArea.label} with topic ${_requestData.conversationArea.topic}`,
